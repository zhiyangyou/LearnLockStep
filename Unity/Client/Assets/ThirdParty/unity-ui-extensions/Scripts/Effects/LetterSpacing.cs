    ...
    public class LetterSpacing : BaseMeshEffect
	{
        ...
        public override void ModifyMesh(VertexHelper vh)
        {
			...
			for (int lineIdx=0; lineIdx < lines.Length; lineIdx++)
			{
                ...
                List<Match> comMatch = null;
                if (bRichText)
                {
                    comMatch = GamePool.ListPool<Match>.Get();
                    for(int j = 0; j < ms_Regexs.Length; ++j)
                    {
                        var matches = ms_Regexs[j].Matches(line);
                        var matchEnumerator = matches.GetEnumerator();
                        while(matchEnumerator.MoveNext())
                        {
                            if((matchEnumerator.Current as Match).Success)
                            comMatch.Add(matchEnumerator.Current as Match);
                        }
                    }
                }

				for (int charIdx = 0,realIndex = 0; charIdx < line.Length; charIdx++)
				{
                    ...
                    if(null != comMatch)
                    {
                        for (int j = 0; j < comMatch.Count; ++j)
                        {
                            if(charIdx >= comMatch[j].Index && charIdx < comMatch[j].Index + comMatch[j].Length)
                            {
                                bFind = true;
                                break;
                            }
                    ...
                    {
                        pos = Vector3.right * (letterOffset * realIndex - lineOffset);
                ...
                if (null != comMatch)
                {
                    GamePool.ListPool<Match>.Release(comMatch);
                    comMatch = null;
                }
