Shader "HeroGo/SimulatePBR/General" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
			...
			float gradualMT;
				...
				wave = frac( wave - _Time.g * 0.7);
				gradualM = abs(wave - 0.5) * 2.0;
				
				float waves = length(localSpace + float3(0, -_GradualLocation, 0.2));
				float ratios = saturate(1 - waves);
				waves = waves * (1 - ratios + 2 * ratios);
				waves = frac( waves - _Time.g * 0.3);
				gradualMT = abs(waves - 0.5) * 2.0;


				//float waves = frac( wave - _Time.g * 0.7);
				//gradualMT = abs(wave - 0.5) * 2.0;
			#endif
                ...
                //o.lightDirection = float4(normalize(/*o.viewDirection + */float3(-0.5, 1, 0.3)), gradualMT);//normalize(_WorldSpaceLightPos0.xyz);
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                fixed4 param = tex2D(_Param, i.uv0);
				...
				fixed Glossing = 1.0 - param.g;
                fixed metallica = param.r;
				fixed3 diffuseColor = mainTex.rgb * _Color.rgb;

		// Diffuse---<
				fixed2 brdfSpace = fixed2(halfOptimize(i.lightDirection, normalVer), halfOptimize(i.viewDirection.xyz, normalVer));
