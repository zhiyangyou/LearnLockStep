...
Shader "HeroGo/General/OneDirLight/Bumped_RimLight_Mono"
{
	Properties
	{
		...
		_RimLightColor("Rim Light Color",Color) = (0.37,0.46,1.0,1.0)
		_RimLightPower("Rim Lgiht Power",Range(0.01,3)) = 1
		_RimLightOffset("Rim Lgiht Offset",Range(-3,3)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				half factor = pow(saturate(rim - _RimLightOffset), _RimLightPower);
				factor *= dot(Nn, normalize(_RimLightDir));
	...
	FallBack "HeroGo/General/OneDirLight/Bumped_RimLight"
}
