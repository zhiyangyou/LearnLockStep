Shader "HeroGo/Dissolution/Rim_Flash_Alpha" {
    Properties {
...
////// Emissive:
				...
				float rim = 1.0 - saturate(dot(i.viewDirection,normalDirection));
				float factor = pow(saturate(rim - _RimLightOffset), _RimLightPower) * i.TintFactor;
				float3 finalRGB = finalColor * (1 - factor) + _RimLightColor.rgb * factor;
