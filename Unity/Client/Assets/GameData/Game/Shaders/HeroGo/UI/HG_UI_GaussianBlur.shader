...
Shader "HeroGo/UI/GaussianBlur"
{
	...
	fixed4 frag_blur(v2f_blur i) : SV_Target
	{
		...
		color.rgb += 0.40 * tex2D(_MainTex, i.uv).rgb;
		color.rgb += 0.15 * tex2D(_MainTex, i.uv01.xy).rgb;
		color.rgb += 0.15 * tex2D(_MainTex, i.uv01.zw).rgb;
		color.rgb += 0.10 * tex2D(_MainTex, i.uv23.xy).rgb;
		color.rgb += 0.10 * tex2D(_MainTex, i.uv23.zw).rgb;
		color.rgb += 0.05 * tex2D(_MainTex, i.uv45.xy).rgb;
		color.rgb += 0.05 * tex2D(_MainTex, i.uv45.zw).rgb;
		color.a = 1;
		return color;
	}
	ENDCG
